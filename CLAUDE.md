...
- WSL 完全対応（承認待ち通知）
  - 現状: `end_turn` のみ検知可能。`tool_use`（承認待ち）は WSL2 の 9P write-back キャッシュにより、`tool_result` 書き込み時にキャッシュがフラッシュされるまで Windows 側から見えない
  - 解決策: Linux 側に `inotifywait` 等でJSONL監視するヘルパーデーモンを置き、TCP で Windows 側 LLMN に通知する仕組みが必要
